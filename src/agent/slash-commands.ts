...
export const SLASH_COMMANDS: SlashCommand[] = [
  ...
  },
  {
    ...
    prompt:
      "Create or revise a table. For a new table, call insert_table with rows, columns, and the full cells 2D array. To edit an existing table, call get_selection first — use table_values for update_table cells. Call list_tables only if table_values is unavailable or multiple tables need comparison. To add rows, pass a longer grid, use insert_rows_at with only the new row(s), or extend a start_row patch past the table end.",
  },
  {
    name: "toc",

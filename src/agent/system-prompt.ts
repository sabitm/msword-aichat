...
export function buildAgentSystemPrompt(
  ...
  const lines = [
    ...
    "- Use update_table to change an existing table in place. Call list_tables first; pass the full cells grid (dimensions are taken from the table). Tables with merged headers (isUniform: false) are updated cell-by-cell — keep header rows identical to list_tables and only change data rows.",

...
export function buildAgentSystemPrompt(
  ...
  const lines = [
    ...
    "- Use find_and_replace to change every occurrence of body text (e.g. dates, names). Skips table cells by default.",
    "- Use replace_at_match to change one specific body-text occurrence. match_index is 0-based (first match is 0, not 1). Not for table cells.",
    "- To fill in table cells or form fields, use get_selection or list_tables for table_values, then update_table. Do not use find_and_replace or replace_at_match for table content.",
